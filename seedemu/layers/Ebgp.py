...
"""
#removed rs client; before local line
...
EbgpFileTemplates["rnode_bird_peer"] = """
    ...
    neighbor {peerAddress} as {peerAsn};
"""
EbgpFileTemplates["rnode_bird_peer2"] = """
    ipv4 {{
        table t_bgp;
        import filter {{
            bgp_large_community.add({importCommunity});
        ...
        export filter {{
                if {exportFilter} then {{
                     accept;
                }}
                if net ~ [ 10.{localAsn}.1.0/24, 10.{localAsn}.2.0/24, 10.{localAsn}.3.0/24, 10.{localAsn}.4.0/24, 10.{localAsn}.5.0/24 ] then {{
                        accept;
                }}
                reject;
        }};
...
class Ebgp(Layer, Graphable):
    ...
    def __createPeer(self, nodeA: Router, nodeB: Router, addrA: str, addrB: str, rel: PeerRelationship) -> None:
        ...
        if rel == PeerRelationship.Peer:
            routerA.addProtocol('bgp', 'p_as{}'.format(routerB.getAsn()), EbgpFileTemplates["rnode_bird_peer2"].format(
                localAddress = addrA,
                localAsn = routerA.getAsn(),
                peerAddress = addrB,
                peerAsn = routerB.getAsn(),
                #exportFilter = "where bgp_large_community ~ [LOCAL_COMM, CUSTOMER_COMM]",
                exportFilter = "bgp_large_community ~ [LOCAL_COMM, CUSTOMER_COMM]",
                importCommunity = "PEER_COMM",
                bgpPref = 20
            ))

            routerB.addProtocol('bgp', 'p_as{}'.format(routerA.getAsn()), EbgpFileTemplates["rnode_bird_peer2"].format(
                localAddress = addrB,
        ...
        if rel == PeerRelationship.Provider:
            ...
            routerB.addProtocol('bgp', 'u_as{}'.format(routerA.getAsn()), EbgpFileTemplates["rnode_bird_peer2"].format(
                localAddress = addrB,
                localAsn = routerB.getAsn(),
                peerAddress = addrA,
                peerAsn = routerA.getAsn(),
