...
class Ospf(Layer):
    ...
    def render(self, emulator: Emulator):
        ...
        for ((scope, type, name), obj) in reg.getAll().items():
            ...
            ospf_interfaces = ''
            #update for if/else statement. Else statement is original. Remove If to undo.
            for name in stubs:
                    ospf_interfaces += OspfFileTemplates['ospf_stub_interface'].format(interfaceName = name)
...
#                   ospf_interfaces += OspfFileTemplates['ospf_stub_interface'].format(interfaceName = name)
#                   router.addTable('t_ospf')
#                   router.addProtocol('ospf', 'ospf2', OspfFileTemplates['ospf_body2'].format(
#                    interfaces = ospf_interfaces
#                ))
#                   router.addTablePipe('t_ospf')
#                else:
