        ...
        for ((scope, type, name), obj) in reg.getAll().items():
            if type == 'rs':
                ...
                rs_node.setFile("/etc/bird/bird.conf", RoutingFileTemplates["rs_bird"].format(
                    routerId = rs_iface.getAddress() #, interfaces=rs_node.getName(), localAsn=rs_node.getAsn() 
                ))
            ...
            if type == 'rnode':
                ...
                ifaces = ''
                has_localnet = False
                ASN_List=[134,135,136, 137, 138, 139, 130]
                ASN_key={134:'ix101', 135:'ix102', 137:'ix104', 136:'ix103', 138:'ix105', 139:'ix106', 130:'ix100'}
                if (rnode.getAsn() in ASN_List) and ASN_key[rnode.getAsn()][2:5] in rnode.getName():
                        has_localnet = True
                        ifaces += RoutingFileTemplates["rnode_bird_direct_interface"].format(
                            interfaceName = ASN_key[rnode.getAsn()])

                for iface in r_ifaces:
                    net = iface.getNet()
                    #if rnode.getAsn() in ASN_List:
                        #has_localnet = True
                        #ifaces += RoutingFileTemplates["rnode_bird_direct_interface"].format(
                            #interfaceName = ASN_key[rnode.getAsn()]
                        #)
